...
public class PlantUserCreateAPI {
    ...
    public void createPlant(int categoryId, Map<String, Object> body) {
        try {
            ...
            System.out.println("DEBUG: PlantUserCreateAPI status=" + statusCode);
            System.out.println("DEBUG: PlantUserCreateAPI body=" + responseBody);

        } catch (Exception e) {
            this.statusCode = -1;
            this.responseBody = e.toString();
            System.out.println("DEBUG: PlantUserCreateAPI exception: " + e.toString());
        } finally {
            if (apiRequest != null) {
                apiRequest.dispose();
            }
